...
BDNS_ALWAYS_OUT_OF_SCOPE_TERMS = (
    ...
    "contratacion de personas", "programas de empleo",
)
...
def _bdns_pre_claude_gate(conv: dict) -> dict | None:
    ...
    cluster_downstream = any(term in folded for term in BDNS_CLUSTER_DOWNSTREAM_TERMS)
    cluster_operations = any(term in folded for term in BDNS_CLUSTER_OPERATING_TERMS)
    supplier_fit = technology_fit and any(term in folded for term in BDNS_SUPPLIER_COST_TERMS)
    ...
    if sections == {"B"} and not manufacturing_evidence:
        return _bdns_gate_result(
    ...
    if sections == {"F"} and not technology_fit:
        return _bdns_gate_result(
            "reject", "building_without_industrial_connection",
            "Construccion sin conexion termica o industrial explicita.", role,
        )
    if sections and sections.isdisjoint(BDNS_POSITIVE_NACE_SECTIONS | {"B", "F"}) and not technology_fit:
        return _bdns_gate_result(
...
def _bdns_detail_to_raw(detail: dict, listing: dict) -> dict | None:
    ...
    if not title or not bdns_id:
        return None
    open_date = _parse_flexible_date(detail.get("fechaInicioSolicitud", ""))
    deadline_date = _parse_flexible_date(detail.get("fechaFinSolicitud", ""))
    ...
    indefinite = explicitly_open or any(term in combined_folded for term in (
    ...
    if deadline_date:
        ...
        received_days = _days_until(received_date) if received_date else -9999
        active_status = "unverified_recent" if -365 <= received_days <= 1 else "unverified_old"
    ...
    if not no_claude:
        try:
            schema_metrics = [
                ...
                for model in (CallFacts, CallEvaluation)
            ]
        except ClaudeAnalysisError as exc:
            print(f"⚠ ERROR: {exc}")
