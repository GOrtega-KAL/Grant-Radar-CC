...
| 40 | **TAREA PENDIENTE, decidida con el usuario el 01/09/2026: aplicar el plural en `_term_present()`.** Hoy exige la frase exacta, así que «calores residuales» no activa `calor residual` ni «tratamientos térmicos» a `tratamiento térmico` | **Ya medido (sección 56.2), no hace falta volver a medirlo**: sobre 368 textos reales, +1 pasa el prefiltro, 0 lo pierden, 0 falsos positivos; unos 0,03 USD por ejecución completa. Las dos coincidencias que se ganan son aciertos, y una es «recuperación de calores residuales», el negocio central del cliente. Implementación: en cada palabra del término, sufijo opcional `(?:e?s)?`. **No ampliar a género** («térmico» → «térmicas») sin medir otra vez. El usuario lo aplazó a una sesión posterior, no lo descartó |
