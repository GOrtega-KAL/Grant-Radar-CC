...
def fetch_programme_eligibility(
    ...
    *,
    ...
    PDF. `cache` es un diccionario por ejecución —una descarga por edición del
    programa, compartida por todos los topics—, no una caché en disco: el
    documento cambia cuando cambia el programa de trabajo, y una ejecución
    completa dura minutos.
    """
    ...
    if response is None or getattr(response, "status_code", 0) != 200:
        log.warning(
            "Anexos Generales no disponibles (%s): la elegibilidad del programa "
            "queda sin declarar", url
        )
    else:
        sections = eligibility_sections(document_text(response, url)[0])
        if sections:
            result = {"source_url": url, **sections}
            log.info(
